...
def index():
    ...
    response = make_response(render_template('interface_nova.html', build_timestamp=BUILD_TIMESTAMP))
...
@require_auth
@require_permission('nfse_view')
def listar_nfse_controle():
    """Lista todas as NFS-e da empresa para o Controle de NFS-e"""
    try:
        import psycopg2
        import psycopg2.extras
        from database_postgresql import get_nfse_db_params
        ...
        logger.error(f"Erro ao listar NFS-e controle: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/nfse/<int:nfse_id>/editar', methods=['PUT'])
@require_auth
@require_permission('nfse_view')
def editar_nfse(nfse_id):
    """Atualiza campos editáveis de uma NFS-e"""
    try:
        import psycopg2
        import psycopg2.extras
        from database_postgresql import get_nfse_db_params
        ...
        campos_editaveis = [
            ...
            'valor_liquido', 'valor_servico', 'discriminacao', 'situacao',
            'situacao_recebimento', 'data_pagamento', 'valor_iss', 'aliquota_iss',
            'valor_deducoes', 'codigo_servico', 'nome_municipio', 'numero_rps',
            'serie_rps', 'protocolo', 'codigo_verificacao',
        ]
        ...
        for campo in campos_editaveis:
            if campo in data:
                val = data[campo]
                # Tratar string vazia como NULL para campos de data/número
                if val == '':
                    val = None
                sets.append(f"{campo} = %s")
                params.append(val)
        ...
        return jsonify({'success': True, 'message': 'NFS-e atualizada com sucesso'})
    except Exception as e:
        logger.error(f"Erro ao editar NFS-e {nfse_id}: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
